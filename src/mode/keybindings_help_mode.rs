...
mod tests {
    ...
    fn test_build_help_rows_contains_archive_picker_search_toggle_entries() {
        ...
        let has_search_merge_toggle = rows.iter().any(|r| {
            matches!(r, HelpRow::Entry { action, keys }
                if action == "Merge mark while searching" && keys == "Alt+m")
        });
        assert!(
            has_search_merge_toggle,
            "expected a 'Merge mark while searching' entry bound to 'Alt+m'"
        );
    }
    ...
    fn test_build_help_rows_contains_archive_picker_search_select_all_entries() {
        ...
        let has_search_merge_all = rows.iter().any(|r| {
            matches!(r, HelpRow::Entry { action, keys }
                if action == "Merge mark all matching search" && keys == "Ctrl+Alt+m")
        });
        assert!(
            has_search_merge_all,
            "expected a 'Merge mark all matching search' entry bound to 'Ctrl+Alt+m'"
        );
    }
