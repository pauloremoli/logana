...
`log_panel.rs::render_template_segments(segments, parts, hidden_fields)` rebuilds the line at render time: each visible field's value comes from `resolve_field(canonical_name, parts)`; a field that's `ignored` or in `hidden_fields` contributes no value, and — unless it's the template's last field — the literal segment immediately following it is dropped too, so hiding a field collapses its separator (`{level}/{component}/{feature}` with `component` hidden renders `INF/StartupMgr`, not `INF//StartupMgr`) instead of leaving a dangling delimiter. `populate_parse_cache` only calls this when `FieldLayout::columns` is `None` (no explicit column *order* set) — a genuine reorder (moving a field with `J`/`K` in `:select-fields`) can't be expressed by the template's fixed separators, so that still falls back to `apply_field_layout`'s column layout. `pattern`-based schemas have no `TemplateSegment`s and always use the column layout.
