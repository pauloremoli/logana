...
src/
  ...
  mode/           - Mode module directory
    mod.rs        - Module declarations
    app_mode.rs   - Mode trait + ModeRenderState enum + KeyResult
    normal_mode.rs - NormalMode: scroll, search, mark, visual, g/G, count prefix
    command_mode.rs - CommandMode: command input with history and tab completion
    filter_management_mode.rs - FilterManagementMode: navigate/toggle/delete/edit filters
    filter_edit_mode.rs - FilterEditMode: edit filter pattern inline
    search_mode.rs - SearchMode: incremental search with live highlighting
    visual_mode.rs - VisualLineMode: line-range selection for bulk mark/comment/yank/filter/search
    visual_char_mode.rs - VisualMode: intra-line character selection with vim motions + display_line_text helper
    comment_mode.rs - CommentMode: multiline comment editor
    keybindings_help_mode.rs - KeybindingsHelpMode: searchable keybindings popup
    select_fields_mode.rs - SelectFieldsMode: checkbox field selector popup
    docker_select_mode.rs - DockerSelectMode: docker container selector popup
    value_colors_mode.rs - ValueColorsMode: per-token color config popup
    ui_mode.rs    - UiMode: display-only toggles (sidebar, bar, borders, wrap)
...
**Visual line mode** (`V`): anchor at current line, j/k extend selection, `c` opens comment editor, `m` marks/unmarks all selected lines (group toggle: all marked → unmark all, else mark all), `y` yanks (copies) selected lines to system clipboard via `arboard`, `i`/`o` pre-fill CommandMode with `"filter <line>"` / `"exclude <line>"`, `/` pre-fills SearchMode with the first selected line, Esc cancel. All keys are configurable via `VisualLineKeybindings`. Selected range highlighted in the log panel.
**Visual char mode** (`v`): character-level selection within the current log line. The cursor moves freely with vim motions (h/l, w/b/e, W/B/E, 0/^/$, f/F/t/T/;/,). Pressing `v` again anchors the selection at the cursor position; subsequent motions extend it. Without an anchor, actions operate on the single char under the cursor. Actions: `i`/`o` pre-fill CommandMode with `"filter <selected>"` / `"exclude <selected>"`, `/` pre-fills SearchMode with selected text, `y` yanks selection to clipboard, Esc cancel. `anchor_col: Option<usize>` is `None` until `v` is pressed; `selection_range()` returns `(cursor, cursor)` when no anchor. `apply_char_selection(line, lo, hi)` overlays `Modifier::REVERSED` on `[lo, hi]` char positions of a rendered `Line`. All keys configurable via `VisualKeybindings`. `display_line_text(tab)` (shared with `normal_mode`) captures the displayed text on entry using parsed field-layout or raw bytes fallback. `search_match_char_offset(tab, line_text)` (in `normal_mode`) pre-positions the cursor at the start of the active search occurrence on the current line when entering visual char mode.
...
- **`Keybindings`**: groups `NavigationKeybindings` (shared scroll/page keys used across all modes), `NormalKeybindings`, `FilterKeybindings`, `GlobalKeybindings`, `CommentKeybindings`, `VisualLineKeybindings`, `VisualKeybindings`, `DockerSelectKeybindings`, `ValueColorsKeybindings`, `SelectFieldsKeybindings`, `HelpKeybindings`, `ConfirmKeybindings`, `UiKeybindings` — each with `#[serde(default)]` so any absent field uses its built-in default. Navigation keys (scroll_down/up, half_page_down/up, page_down/up) are configured once in the `navigation` group and shared by all modes. `VisualKeybindings` covers all char-mode motions (move_left/right, word_forward/backward/end, big variants, start_of_line, first_nonblank, end_of_line, find_forward/backward, till_forward/backward, repeat_motion/rev) and actions (start_selection, filter_include, filter_exclude, search, yank, exit).
...
- **Instant preview**: before spawning the background index job, `begin_file_load` reads 16 MiB synchronously (`from_file_tail` for `--tail`, `from_file_head` otherwise) and populates the initial tab immediately. When a filter predicate is active it is applied to the preview lines so a filtered view is visible straight away — the full-file filtered result arrives later via the background job. The background job replaces the preview reader with the complete index when done.
