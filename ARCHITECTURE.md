...
**Persistence** (`db/`) — `Database` owns the SQLite connection pool and schema migrations. `Database::open`/`in_memory` run pragmas and every `migrate_to_vN` step on one dedicated connection acquired up front, rather than letting each statement round-trip through the pool independently — schema-changing statements (especially the `filters` table rebuild in `migrate_to_v12`) must not be interleaved with another pooled connection observing a stale view of the schema. Storage is accessed through four traits: `FilterStore` (filter definitions), `FileContextStore` (per-file scroll/search/display context), `SessionStore` (open file list), and `AppSettingsStore` (runtime toggles keyed by `SettingsKey`). `LogManager` builds `FilterManager` instances from persisted filter definitions. `MarkManager` and `CommentManager` are in-memory managers owned by `TabState`; their state is flushed to SQLite via `FileContext` on tab close/switch. Session save/restore is coordinated by `SessionManager` in `ui/session.rs`.
...
For a `template`-based schema, `CustomParser` also compiles the template into an ordered `Vec<TemplateSegment>` (`parser::types::TemplateSegment`: `Literal(String)` | `Field { canonical_name, ignored }`) alongside the regex, exposed via `LogFormatParser::template_segments()`. Each `Field` carries the *canonical* name (`resolve_field`/`hidden_fields` use, e.g. a `{service}` placeholder mapped to the `target` role is stored as `canonical_name: "target"`), not the raw template placeholder — resolved once at `from_config` time from `field_map`.

`field_layout.rs::render_template_segments(segments, parts, hidden_fields)` rebuilds the line at render time: each visible field's value comes from `resolve_field(canonical_name, parts)`; a field that's `ignored` or in `hidden_fields` contributes no value, and — unless it's the template's last field — the literal segment immediately following it is dropped too, so hiding a field collapses its separator (`{level}/{component}/{feature}` with `component` hidden renders `INF/StartupMgr`, not `INF//StartupMgr`) instead of leaving a dangling delimiter. It's gated behind `reconstructed_line_text(parser, parts, field_layout, hidden_fields)`, which returns `None` (falling back to `apply_field_layout`'s column layout) when `FieldLayout::columns` is `Some(..)` (an explicit reorder — moving a field with `J`/`K` in `:select-fields` — can't be expressed by the template's fixed separators) or the parser has no `TemplateSegment`s (`pattern`-based schemas). `render_line_text(parser, parts, field_layout, hidden_fields, show_keys, year_override)` wraps that with the `apply_field_layout` fallback into the single function that decides what a line's text actually is. Every consumer that needs "the text of this line, as it's actually shown" goes through it (or the equivalent `TabState::get_display_text`, which adds the `raw_mode` check and the same `apply_field_layout`-empty-vec-falls-back-to-raw-bytes edge case): `log_panel.rs::populate_parse_cache` (what's drawn), `visual_char_mode.rs::display_line_text` (word motions), `TabState::get_display_text`/`display_text_for_line` (search match offsets, and `yy`/`Y`/visual-line yank via `TabState::get_display_text`), and `export.rs::render_line_content` (`:export`) — so none of them can disagree with each other about what's on screen.

`CustomParser::collect_field_names` (which seeds the `:select-fields` popup's initial list, and doubles as `SelectFieldsMode`'s `default_order` for its `r`/reset key) reuses the same `template_segments` — for a `template`-based schema it returns each non-`ignored` field's canonical name in template order, not the generic timestamp/level/target/sorted-extras/message grouping the fallback (and every other parser) uses, so the popup's order always matches what the line actually renders as by default.

`select_fields_mode.rs`'s apply step only pins `field_layout.columns` to `Some(..)` when the staged order differs from `default_order` — matching it (whether nothing was ever touched, or the `r` key reset it) leaves `columns` at `None`, so it doesn't disable template reconstruction by itself. `cmd_show_all_fields` (`:show-all-fields`) clears both `hidden_fields` and `field_layout.columns` for the same reason.
...
Each text filter (the `Text` box above) can be marked `ignore_case` (`FilterDef::ignore_case`, set via `--ignore-case`/`-i`). `SubstringFilter`/`RegexFilter` bake this into their own matcher at construction (`AhoCorasick::ascii_case_insensitive` / `RegexBuilder::case_insensitive`). Since `aho-corasick` case-insensitivity is a whole-automaton setting, `LogManager::build_filter_manager` can only merge same-case-sensitivity literal patterns into the shared `combined_ac` fast-path automaton; a case-insensitive literal filter (like a regex filter) instead goes into `FilterManager::slow_path_filter_indices` and is evaluated via its own compiled matcher.
